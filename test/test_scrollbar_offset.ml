...
let%expect_test "vertical" =
  ...
  test_vert ~rows ~cols (fun () ->
    let scrollbar, remaining =
      Scroll.VScrollbar.create_right_aligned ~container ~range ()
    in
    Scroll.Scrollbar.set_offset_min scrollbar;
    scrollbar, remaining);
  [%expect
    {|
    ##############################################################
  ...
  test_vert ~rows ~cols (fun () ->
    let scrollbar, remaining =
      Scroll.VScrollbar.create_right_aligned ~container ~range ()
    in
    Scroll.Scrollbar.set_offset_max scrollbar;
    scrollbar, remaining);
  [%expect
    {|
    ##############################################################
...
let%expect_test "horizontal" =
  ...
  test_horz ~rows ~cols (fun () ->
    let scrollbar, remaining =
      Scroll.HScrollbar.create_bottom_aligned ~container ~range ()
    in
    Scroll.Scrollbar.set_offset_min scrollbar;
    scrollbar, remaining);
  [%expect
